<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_subscriptions_table">
    ...
    <tbody class="text-gray-600 fw-semibold">
        @if(isset($permissionsList) && count($permissionsList) > 0)
            @foreach($permissionsList as $keys => $singleRec)
                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td class="py-4 px-6 text-sm font-medium text-gray-500 whitespace-nowrap dark:text-white">{{ $singleRec->name }}</td>
        ...
        @else
            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 text-center">
                <td colspan="10">Permissions list not found.</td>
            </tr>
        @endif
    </tbody>
</table>
{!! $permissionsList->links('pagination') !!}