@php
    ...
    $name = $userDetails ? $userDetails->name : old('name');
    $email = $userDetails ? $userDetails->email : old('email');
    $country_id = $userDetails ? $userDetails->country_id : old('country_id');
    $state_id = $userDetails ? $userDetails->state_id : old('state_id');
    $city_id = $userDetails ? $userDetails->city_id : old('city_id');
    $pincode = $userDetails ? $userDetails->pincode : old('pincode');
    $address = $userDetails ? $userDetails->address : old('address');

    $roles = [];
    if(count($userDetails->roles) > 0)
    {
        foreach ($userDetails->roles as $key => $value) {
            $roles[] = $value->id;
        }
    }

    $bankId = $userDetails ? $userDetails->bankAccount->id : '';
    $branchName = $userDetails ? $userDetails->bankAccount->branch_name : old('branch_name');
    $bankName = $userDetails ? $userDetails->bankAccount->bank_name : old('bank_name');
    $accountNumber = $userDetails ? $userDetails->bankAccount->account_number : old('account_number');
    $IFSCCode = $userDetails ? $userDetails->bankAccount->ifsc_code : old('ifsc_code');
    $ACHolderName = $userDetails ? $userDetails->bankAccount->ac_holder_name : old('ac_holder_name');
@endphp
<x-tenant-app-layout>
                    ...
                    <div class="col-xl-12">
                        <div class="card card-flush h-lg-100" id="kt_contacts_main">
                            ...
                            <div class="card-body pt-5">
                                <form method="POST" class="form" action="{{ route('users.update', $userDetails->id) }}" id="userForm" enctype='multipart/form-data'>
                                    @csrf
                                    @method('PUT')
                                    <input type="hidden" name="user_id" id="user_id" value="{{ $userId }}" />
                                    <input type="hidden" name="bank_id" id="bank_id" value="{{ $bankId }}" />
                                    ...
                                    <div class="mb-7">
                                        ...
                                        <div class="mt-1">
                                            <style>.image-input-placeholder { background-image: url('{{ $profilePic }}' ) } [data-bs-theme="dark"] .image-input-placeholder { background-image: url('{{ asset_url("app/media/svg/files/blank-image-dark.svg") }}'); }</style>
                                    ...
                                    <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-2">
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="$name" placeholder="Name" required autocomplete="username" />
                                        ...
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="email" class="block mt-1 w-full" type="text" name="email" :value="$email" placeholder="Email" required />
                                    ...
                                    <div class="fv-row mb-7">
                                        ...
                                        <textarea class="form-control form-control-solid" name="address" required>{{ $address }}</textarea>
                                    </div>
                                    <div class="fv-row mb-7">
                                        <label class="fs-6 fw-semibold form-label mt-3">
                                        ...
                                        <select class="form-control mt-1" id="user_role" name="user_role">
                                            ...
                                            @if(count($getAllRole) > 0)
                                                @foreach($getAllRole as $role)
                                                    <option value="{{ $role->id }}" {{ in_array($role->id, $roles) ? 'selected' : '' }}>{{ $role->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                                    ...
                                    <div class="row row-cols-1 row-cols-sm-2 rol-cols-md-1 row-cols-lg-2">
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="branch_name" class="block mt-1 w-full" type="text" name="branch_name" :value="$branchName" placeholder="Branch Name" required />
                                        ...
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="bank_name" class="block mt-1 w-full" type="text" name="bank_name" :value="$bankName" placeholder="Bank Name" required />
                                        ...
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="account_number" class="block mt-1 w-full" type="text" name="account_number" :value="$accountNumber" placeholder="Account Number" required />
                                        ...
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="ifsc_code" class="block mt-1 w-full" type="text" name="ifsc_code" :value="$IFSCCode" placeholder="IFSC Code" required />
                                        ...
                                        <div class="col">
                                            <div class="fv-row mb-7">
                                                ...
                                                <x-text-input id="ac_holder_name" class="block mt-1 w-full" type="text" name="ac_holder_name" :value="$ACHolderName" placeholder="A/C Holder Name" required />
                            ...
                            success: function(response) {
                                ...
                                if(response.status)
                                {
                                    ...
                                    $.each(response.data, function(index, item) {
                                        ...
                                        var selected = stateId == item.id ? 'selected' : '';
                                        state += "<option value='" + item.id + "' " + (selected ? 'selected' : '') + ">" + item.name.charAt(0).toUpperCase() + item.name.slice(1) + "</option>";
                                    });
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if(response.status)
                                {
                                    ...
                                    $.each(response.data, function(index, item) {
                                        // Access properties of each object
                                        var selected = cityId == item.id ? 'selected' : '';
                                        city += "<option value='" + item.id + "' " + (selected ? 'selected' : '') + ">" + item.name.charAt(0).toUpperCase() + item.name.slice(1) + "</option>";
                                    });
                ...
                $.ajax({
                    ...
                    success:function(response)
                    {
                        ...
                        if(response.status)
                        {
                            ...
                            $.each(response.data, function(index, item) {
                                // Access properties of each object
                                var selected = countryId == item.id ? 'selected' : '';
                                country += "<option value='" + item.id + "' " + (selected ? 'selected' : '') + ">" + item.name.charAt(0).toUpperCase() + item.name.slice(1) + "</option>";
                            });
