<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_subscriptions_table">
    ...
    <tbody class="text-gray-600 fw-semibold">
        @if(isset($categoryList) && count($categoryList) > 0)
            @foreach($categoryList as $keys => $singleRec)
                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                    ...
                    <td class="py-4 px-6 text-sm font-medium text-gray-500 whitespace-nowrap dark:text-white">{{ $singleRec->category_name }}</td>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton_{{ $keys }}">
                                ...
                                <li><a class="dropdown-item" onclick="editCategory('{{ $singleRec->category_name }}','{{ encrypt($singleRec->id) }}')" href="#">Edit</a></li>
                                <li>
                                    <a class="dropdown-item" onclick="deleteCategory('{{ encrypt($singleRec->id) }}')" href="#">Delete</a>
                                </li>
                            </ul>
                        </div>
                    </td>
        ...
        @else
            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700 text-center">
                <td colspan="10">Categories list not found.</td>
            </tr>
        @endif
    </tbody>
</table>
{!! $categoryList->links('pagination') !!}